...
import { writeRuntimeAssets } from '../../shared/runtime-assets.mjs'
...
await copyFile(join(library, 'opencv.mjs'), join(root, 'public/runtime', `opencv-${manifest.sha256}.mjs`))
await mkdir(join(root, 'src'), { recursive: true })
await writeFile(join(root, 'src/wasm.generated.json'), JSON.stringify(manifest, null, 2) + '\n')
...
await writeFile(join(root, 'public/samples.json'), JSON.stringify(samples) + '\n')
