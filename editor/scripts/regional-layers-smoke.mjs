...
try {
  ...
  assert.match(completionSummary, /pink isolated holes; blue temporal holes/)
  ...
  assert.match(completionSummary, /Cleanup: isolated on; temporal on/)
  const isolatedToggle = page.getByLabel('Support Completion Fill isolated holes', { exact: true })
  const temporalToggle = page.getByLabel('Support Completion Bridge temporal holes', { exact: true })
  assert.equal(await isolatedToggle.isChecked(), true); assert.equal(await temporalToggle.isChecked(), true)
  await change(() => isolatedToggle.uncheck())
  assert.match(await page.locator('.value-preview pre').innerText(), /Cleanup: isolated off; temporal on/)
  await change(() => temporalToggle.uncheck())
  assert.match(await page.locator('.value-preview pre').innerText(), /isolated 0; temporal 0/)
  assert.match(await page.locator('.value-preview pre').innerText(), /Cleanup: isolated off; temporal off/)
  await change(() => isolatedToggle.check())
  await change(() => temporalToggle.check())
