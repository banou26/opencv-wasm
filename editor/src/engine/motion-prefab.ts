...
const workingPair = (): NodeDefinition => {
  const definition: NodeDefinition = { id: 'gworkingpair', name: 'Working frame pair', description: 'Limit the longest side to Max side without enlarging the input. Preserve aspect ratio and round dimensions down to even sizes for MP4 output. Resize both frames to those same dimensions. Motion analysis uses these working-image pixels. Draw Motion Vectors maps the results onto the original frame for full-resolution output. Open the group to change the sizing arithmetic.', inputs: [framePort('a', 'Before'), framePort('b', 'After'), number('limit', 'Max side · px', 640)], outputs: [framePort('a', 'Before · resized'), framePort('b', 'After · resized')], graph: { version: 1, nodes: [], edges: [] } }
...
export const motionVectorsGraph = (): GraphDocument => {
  ...
  w('na', 'out:frame:image', 'ndraw', 'in:frame:image'); w('ngrid', 'out:flow:field', 'ndraw', 'in:flow:field'); w('ngrid', 'out:frame:image', 'ndraw', 'in:frame:mask'); w('ndraw', 'out:frame:image', 'n5', 'in:frame:image')
  return b.doc
}
