...
const border: Parameter = { kind: 'select', key: 'border', label: 'Border sampling', default: 'constant', options: ['constant', 'reflect', 'wrap', 'replicate'] }
const BASE: Partial<Record<NodeType, NodeSpec>> = {
  ...
  translateX: { type: 'translateX', title: 'Translate X', category: 'Transform', description: 'Shift an image horizontally by a scalar number of pixels. Positive values move right. Constant borders are black; reflect, wrap or replicate controls sampling beyond the image.', algorithm: 'warpAffine · X axis', version: 1, inputs: [{ id: 'in:frame:image', label: 'Image', type: 'frame' }, { id: 'in:scalar:pixels', label: 'X pixels · optional', type: 'scalar', optional: true }], outputs: [{ id: 'out:frame:image', label: 'Image', type: 'frame' }], parameters: [{ kind: 'number', key: 'pixels', label: 'Fallback X · px', default: 0, min: -1000000, max: 1000000, step: 0.1 }, border] },
  translateY: { type: 'translateY', title: 'Translate Y', category: 'Transform', description: 'Shift an image vertically by a scalar number of pixels. Positive values move down. Constant borders are black; reflect, wrap or replicate controls sampling beyond the image.', algorithm: 'warpAffine · Y axis', version: 1, inputs: [{ id: 'in:frame:image', label: 'Image', type: 'frame' }, { id: 'in:scalar:pixels', label: 'Y pixels · optional', type: 'scalar', optional: true }], outputs: [{ id: 'out:frame:image', label: 'Image', type: 'frame' }], parameters: [{ kind: 'number', key: 'pixels', label: 'Fallback Y · px', default: 0, min: -1000000, max: 1000000, step: 0.1 }, border] },
  ...
  grayscale: { type: 'grayscale', title: 'Grayscale', category: 'Color', description: 'Keep brightness while removing color. Useful before comparing structure or motion.', algorithm: 'transform · cvtColor', version: 1, inputs: [{ id: 'in:frame:image', label: 'Image', type: 'frame' }], outputs: [{ id: 'out:frame:image', label: 'Image', type: 'frame' }], parameters: [{ kind: 'select', key: 'weights', label: 'Weights', default: 'rec709', options: ['rec709', 'rec601', 'average'] }] },
...
export const PORT_LABELS: Record<PortType, string> = { frame: 'Frame', video: 'Video', scalar: 'Number', boolean: 'Boolean', string: 'Text', vector: 'Vector 2D', rect: 'Rectangle', frames: 'Frame list', custom: 'Record', flow: 'Flow field', motion: 'Motion', regions: 'Regions' }
...
export const specFor = (node: GraphNode, doc: GraphDocument): NodeSpec => {
  if (node.type === 'regionalInspect') {
    const titles: Record<string, string> = { source: 'Scene Source', flow: 'Dense Motion', validity: 'Motion Validity', cells: 'Motion Cells', tracks: 'Motion Groups', families: 'Motion Families', velocities: 'Velocity Histories', conflicts: 'Motion Conflicts', completion: 'Support Completion', events: 'Drawing Events', timeline: 'Timing Timeline', review: 'Regional Review' }
