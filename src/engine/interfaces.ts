...
export const updateInterface = (root: GraphDocument, id: string, patch: Pick<NodeDefinition, 'name' | 'inputs' | 'outputs'>): GraphDocument => {
  ...
  const clean = (body: GraphDocument, interfaceId?: string): GraphDocument => {
    let view: GraphDocument = { ...body, definitions, ...(interfaceId ? { interfaceId } : {}) }
    view = { ...view, nodes: view.nodes.map(n => n.type === 'group' && n.definition === id ? { ...n, params: Object.fromEntries(specFor(n, view).parameters.map(p => [p.key, n.params[p.key] ?? p.default])) } : n) }
