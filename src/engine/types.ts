...
export type NodeType = 'source' | 'grayscale' | 'blur' | 'delta' | 'output'
/** Serializable parameters; finite numbers only. */
export type Params = Record<string, string | number | boolean>
/** A typed socket; offsetParam requests a different upstream frame. */
export type Port = { id: string; label: string; type: PortType; offsetParam?: string }
...
export type GraphNode = { id: string; type: NodeType; params: Params; position: { x: number; y: number } }
...
export type GraphDocument = { version: 1; nodes: GraphNode[]; edges: GraphEdge[] }
