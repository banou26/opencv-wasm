...
export type WorkerEvent =
  ...
  | { type: 'thumbnail'; generation: number; path?: string[]; node: string; frame: number; bitmap?: ImageBitmap; scalar?: number; error?: string }
  | { type: 'source'; request: number; value: SourceInfo }
  | { type: 'status'; request: number; value: NodeStatus }
  | { type: 'result'; request: number; selected: string; path?: string[]; frame: number; width: number; height: number; scalar?: number; motion?: { dx: number; dy: number; response: number }; range: 'unit' | 'signed'; elapsed: number; cacheBytes: number; cacheEntries: number }
