...
export async function writeRuntimeAssets(library, destination) {
  const binary = await fs.readFile(path.join(library, 'opencv_js.wasm'))
  const sha256 = createHash('sha256').update(binary).digest('hex')
  const chunkSize = 16 * 1024 * 1024
  await fs.rm(destination, { recursive: true, force: true })
  await fs.mkdir(destination, { recursive: true })
  for (const name of (await fs.readdir(library)).filter(name => /\.(?:js|mjs)$/.test(name))) {
  ...
  for (let offset = 0; offset < binary.byteLength; offset += chunkSize) {
    ...
    chunks.push({ url: `/runtime/${name}`, byteLength: bytes.byteLength })
  }
