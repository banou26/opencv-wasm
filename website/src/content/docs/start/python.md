...
| --- | --- |
| `gray = cv2.cvtColor(image, code)` | `using gray = new cv.Mat(); cv.cvtColor(image, gray, code)` |
| `np.array(...)` | `matFromArray(cv, rows, cols, type, typedArray)` |
...
| `cv2.imread(hostPath)` | Read bytes yourself and `decodeImage`, or put a file in `cv.FS` |
...
```ts
using detector = cv.SIFT_create()
if (!detector) throw new Error('SIFT was not created')
using points = new cv.KeyPointVector()
using descriptors = new cv.Mat()
using noMask = new cv.Mat()
